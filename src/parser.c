...
int
parser_init (parser_t *self, const char *filename)
{
    self->syms = (symbols_t){ .cnt = 0, .m = { 0 } };

    rules_init (&self->rules);
...
parser_destroy (parser_t *self)
{
    ...
    rules_destroy (&self->rules);
    scanner_destroy (&self->scanner);
}

// Stmt = [ newline | endoffile | comment ] .
//      | [ endoffile ] .
//      | [ comment ] .
//      | [ param param param param param command [arg...] ] .
static int
stmt (parser_t *self)
{
    size_t i = 0;
